...
print(Scanner.is_valid('01-03-2019'))
new_scanner = Scanner('Сканер', 0.8, '8000 руб', False, "01-03-2019")
print(new_scanner.validator())
new_xerox = Xerox('Ксерокс 2-го поколения', 4, '34000 руб', False, 2)
try:
    new_warehouse.acceptance(printer_with_ink, int(input()))
    new_warehouse.acceptance(printer_without_ink, int(input()))
    new_warehouse.acceptance(new_scanner, int(input()))
    new_warehouse.acceptance(new_xerox, int(input()))
    new_warehouse.acceptance(new_xerox, int(input()))
    new_warehouse.acceptance(new_xerox, int(input()))
    print(new_warehouse)
    print()
    new_warehouse.transfer(new_scanner, int(input()), 'Бухгалетрия')
    new_warehouse.transfer(printer_without_ink, int(input()), 'Тех. отдел')
    new_warehouse.transfer(new_xerox, int(input()), 'Менеджмент')
    print(new_warehouse)
except ValueError:
    print(IntTypeErr())
    print(new_warehouse)